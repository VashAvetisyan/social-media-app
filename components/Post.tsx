...
export default function Post({ data, id }: postProps) {
	...
	return (
		<div className='border-b border-e-gray-100'>
			<Link href={'/' + id}>
				...
				<div className='relative'>
					{
						data.likes.includes(user.uid) ?
							<HeartSolidIcon className='w-[22px] h-[22px] cursor-pointer text-pink-500 transition' onClick={() => likePost()} />
							:
							<HeartIcon className='w-[22px] h-[22px] cursor-pointer hover:text-pink-500 transition' onClick={() => likePost()} />
					}
...
export function PostHeader({ username, name, timestamp, text, replyTo }: PostHeaderProps) {
	return (
		<div className='flex p-3 space-x-5'>
			<Image src="/assets/profile.png" width={44} height={44} alt='profile' className='w-11 h-11 z-10 bg-white' />

			<div className='text-[15px] flex flex-col space-y-1.5'>
				<div className='flex space-x-1.5 text-[#707E89]'>
					<span className='inline-block font-bold tex-[#BF1419] whitespace-nowrap overflow-hidden text-ellipsis max-w-[60px] min-[400px]:max-w-[100px] min-[500px]:max-w-[140px] sm:max-w-[160px]'>{name}</span>
					<span className='inline-block whitespace-nowrap overflow-hidden text-ellipsis max-w-[60px] min-[400px]:max-w-[100px] min-[500px]:max-w-[140px] sm:max-w-[160px]'>@{username}</span>
